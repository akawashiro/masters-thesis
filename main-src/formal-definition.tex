...
Algorithmic typing rules are rules for implment type checker which takes an
environment \( \Gamma \), a signature \( \Sigma \), a term \( M \), a type \(
tau \) and a stage \( A \) and returns \( \G \V M : \tau @ A \) or not.

Algorithmic typing are almost the same with normal typing rules which we
introduced in Section \ref{sec:typing} so we will explain only some different
points.

There are no coversion rules such as \TConv and \KConv.
