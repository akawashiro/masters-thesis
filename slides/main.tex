...
\begin{frame}[fragile]{他段階計算とはなにか}
...
\begin{frame}[fragile]{MetaOCamlによる他段階計算の例}
    \begin{exampleblock}{}
...
\begin{frame}[fragile]{他段階計算の応用例: vadd-gen}
    特定の長さのベクトルに特化したベクトルの和を計算する関数を作る
    \begin{exampleblock}{}
    \begin{Verbatim}[numbers=left]
...
\begin{frame}[fragile]{依存型[Martin-L{\"o}f'73]を用いた解決}
    \renewcommand{\V}{\text{vec}\ }
    依存型とは値に依存する型
    \begin{itemize}
        \item {\verb|vadd3|} 関数の誤使用を防ぐことができる
    \end{itemize}
    \begin{exampleblock}{}
 \begin{Verbatim}[numbers=left]
 [1;2]: vec 2
...
\begin{frame}[fragile]{本研究の貢献: \LMD}
    既存の他段階計算体系 \LTP [Hanada\&Igarashi'14] を拡張して
    依存型付他段階計算体系 \LMD を設計した
    \begin{itemize}
        \item 他段階計算の機能
        \item 依存型の機能
    \end{itemize}
    ...
    \begin{itemize}
        \item \LMD の型付け規則の設計
        \item 型健全性の証明
        \item 型推論アルゴリズムの設計と完全性・健全性の証明
    \end{itemize}
\end{frame}

\section{既存の他段階計算体系 \LTP}
...
\begin{frame}[fragile]{Cross Stage Persistence (CSP)}
    低いステージの項をより高いステージの中に埋め込むことを可能にする \\[3mm]
