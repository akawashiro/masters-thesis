...
\begin{frame}[fragile]{vadd3の技術的な問題}
    ...
    \begin{exampleblock}{2つのベクトルを足し合わせて2倍する関数のコード}
        ...
        \hspace{1mm}\onslide<2->{\xmark}\hspace{10mm} $\TBL${\tt double-code }($\TBL$\verb|vadd3 | \verb|v | \verb|w|))
    \end{exampleblock}
    吹き出し「\verb|vec 3 -> vec 3|」\\
    吹き出し[\verb|vec %3|」\\
    \onslide<2->{型が合わないので組み合わせることができない}
\end{frame}
...
\begin{frame}[fragile]{\LMD の性質}
    ...
    \end{theorem}
    \begin{theorem}[合流性]
        \(\Gamma \vdash M : \tau \)であり、項\(M\)を実行して2つの異なる項\(M'\)と\(M''\)を得たとき、適切に\(M'\)と\(M''\)を実行すれば同一の項\(M'''\)にたどり着く
    \end{theorem}
\end{frame}
