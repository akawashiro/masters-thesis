...
\begin{frame}[fragile]{多段階計算の応用例: vadd-gen}
    ...
    \end{exampleblock}
        ...
        \begin{tikzpicture}[remember picture, overlay]
            \node[rectangle callout, fill=red!80, white, callout absolute pointer={(unrolled.south)}, below=0.2cm of unrolled.south] {ループ展開による高速化};
        \end{tikzpicture}}
\end{frame}
...
\begin{frame}[fragile]{ステージを跨ぐコードの埋め込み(CSP)}
    ...
    \begin{exampleblock}{}
        \hspace{5mm} $\lambda$\tikz[remember picture, baseline=(stage0.base)]{\node (stage0){\alt<2->{\underline{\tt x:int}}{\tt x:int}};}{\tt .(}$\TB${\tt (}\tikz[remember picture, baseline=(stage1.base)]{\node (stage1){\alt<3->{\underline{\tt \%x + 1}}{\tt \%x + 1}};}{\tt ))}
        \onslide<2->{
        \begin{tikzpicture}[remember picture, overlay]
            \node[rectangle callout, fill=red!80, white, callout absolute pointer={(stage0.south)}] at (-5.4,-2.5) {xはステージ0の項};
        \end{tikzpicture}}
    \onslide<3->{
        \begin{tikzpicture}[remember picture, overlay]
            \node[rectangle callout, fill=red!80, white, callout absolute pointer={(stage1.south)}] at (-1,-1) {ここはステージ1なので\%が必要};
        \end{tikzpicture}}
    \end{exampleblock}
\end{frame}
...
\begin{frame}[fragile]{vadd3の技術的な問題}
    ...
    \begin{alertblock}{2つのベクトルを足し合わせて2倍する関数のコード}
        ...
        \hspace{1mm}\onslide<2->{\xmark}\hspace{10mm} \tikz[remember picture, baseline=(double.base)]{\node (double){\alt<2->{\underline{$\TBL${\tt double-code}}}{$\TBL${\tt double-code }}}}(\tikz[remember picture, baseline=(vadd3.base)]{\node (vadd3){\alt<3->{\underline{$\TBL${\tt vadd3}}}{$\TBL${\tt vadd3 }}}} \verb|v | \verb|w|))
    \end{alertblock}
        \onslide<2->{
        \begin{tikzpicture}[remember picture, overlay]
    ...
    \onslide<3->{
        ...
        \end{tikzpicture}}
    \onslide<4->{\begin{alertblock}{}型が合わないので組み合わせることができない\end{alertblock}}
\end{frame}
